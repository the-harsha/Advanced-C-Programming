...
//A Task in .NET represents an operation that can run asynchronously (non-blocking) or synchronously (blocking).
//Task.Run() is used to run CPU-bound work on a ThreadPool worker thread.
...
namespace FinancialTradingPlatformApplication
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // Initialize class that represents historical market data
            ...
            /* ASYNCHRONOUS EXECUTION USING Task.Run()
            Each method is wrapped in Task.Run() which runs the workload
            on a ThreadPool worker thread, allowing the main thread to continue */
            ...
            // Add each Task to the list so we can wait for all
